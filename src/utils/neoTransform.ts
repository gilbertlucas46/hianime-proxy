...
export const allowedExtensions = ['.ts', '.png', '.jpg', '.webp', '.ico', '.html', '.js', '.css', '.txt', '.key', '.mp4', '.mkv', '.avi', '.vtt', '.srt', '.ass'];

export class EncodedLineTransform extends Transform {
    ...
    constructor(baseUrl: string, originalHeaders?: string) {
        ...
        console.log(`Encoded Transform: Base URL set to: ${this.baseUrl}`);
    }
    ...
    _transform(chunk: Buffer, encoding: BufferEncoding, callback: TransformCallback) {
        ...
        if (!this.isMasterPlaylist) {
            ...
            console.log(`Encoded Transform: Detected ${this.isMasterPlaylist ? 'master' : 'media'} playlist`);
        }
    ...
    private createProxyUrl(url: string): string {
        ...
        // Check if this is an encoded URL format
        const encodedMatch = url.match(/^https?:\/\/[^\/]+\/([^.]+)\.m3u8$/);
        if (encodedMatch) {
            return `/encoded-proxy?url=${encodeURIComponent(url)}${separator}${this.originalHeaders}`;
        }
        
        // Default to encoded proxy for all URLs
        return `/encoded-proxy?url=${encodeURIComponent(url)}${separator}${this.originalHeaders}`;
    }
    ...
    private processLine(line: string): string {
        ...
        if (line.includes('#EXT-X-KEY:METHOD=AES-128,URI=')) {
            ...
            if (match && match[1]) {
                ...
                if (keyPath.startsWith('http')) {
                    ...
                    console.log(`Encoded Transform: Constructed key URL: ${keyUrl}`);
                } else {
                    keyUrl = this.baseUrl + keyPath;
                }

                console.log(`Encoded Transform: Proxying key: ${keyUrl}`);
                return line.replace(
                    `URI="${keyPath}"`,
        ...
        if (line.includes('#EXT-X-I-FRAME-STREAM-INF') && line.includes('URI=')) {
            ...
            if (match && match[1]) {
                ...
                console.log(`Encoded Transform: Proxying I-frame stream: ${iframePath} → ${iframeUrl}`);
                return line.replace(
                    `URI="${iframePath}"`,
        ...
        if (line.includes('#EXT-X-MEDIA') && line.includes('URI="')) {
            ...
            if (match && match[1]) {
                ...
                console.log(`Encoded Transform: Proxying media track: ${mediaPath} → ${mediaUrl}`);
                return line.replace(
                    `URI="${mediaPath}"`,
        ...
        if (this.isMasterPlaylist && !line.startsWith('#') && line.includes('.m3u8')) {
            if (line.includes('/encoded-proxy?url=')) {
                return line;
            }
            ...
            const streamUrl = line.startsWith('http')
                ? line
                : this.baseUrl + line;
            console.log(`Encoded Transform: Proxying video stream: ${line} → ${streamUrl}`);
        ...
        if (!this.isMasterPlaylist && !line.startsWith('#') && line.endsWith('.ts')) {
            const segmentUrl = line.startsWith('http')
                ? line
                : this.baseUrl + line;
            console.log(`Encoded Transform: Proxying segment: ${line} → ${segmentUrl}`);
        ...
        if (!line.startsWith('#') && 
            ...
            const subtitleUrl = line.startsWith('http')
                ? line
                : this.baseUrl + line;
            console.log(`Encoded Transform: Proxying subtitle file: ${line} → ${subtitleUrl}`);
        ...
        if (!line.startsWith('#') && allowedExtensions.some(ext => line.endsWith(ext))) {
            const fileUrl = line.startsWith('http')
                ? line
                : this.baseUrl + line;
            console.log(`Encoded Transform: Proxying static file: ${line} → ${fileUrl}`);
